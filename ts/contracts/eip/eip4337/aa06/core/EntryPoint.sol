...
contract EntryPoint is IEntryPoint, StakeManager, NonceManager, ReentrancyGuard {
     ...
     */
    function _executeUserOp(uint256 opIndex, UserOperation calldata userOp, UserOpInfo memory opInfo) internal returns (uint256 collected) {
    ...
    function _createSenderIfNeeded(uint256 opIndex, UserOpInfo memory opInfo, bytes calldata initCode) internal {
        if (initCode.length != 0) {
            ...
            address sender1 = SenderCreator(address(0x7fc98430eAEdbb6070B35B39D798725049088348)).createSender{gas : opInfo.mUserOp.verificationGasLimit}(initCode);
     ...
     */
    function _handlePostOp(uint256 opIndex, IPaymaster.PostOpMode mode, UserOpInfo memory opInfo, bytes memory context, uint256 actualGas) internal returns (uint256 actualGasCost) {
        uint256 preGas = gasleft();
        unchecked {
